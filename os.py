...
def run_script(command):
    try:
        ...
        base_dir = "voidos/data/0"
        script_path = os.path.join(base_dir, script_name)

        # Controleer of het script in /voidos/data/0 staat
        if not script_path.startswith(base_dir):
            print("Access denied: Only scripts from /voidos/data/0 are allowed")
            return
...
def update():
    os.system("python voidos/updater.py")
